``floo`` - decentralized & secure synchronization
=================================================
...
``floo`` is a distributed & secure file synchronization tool with version
...
* No central server at all. Still, central architectures can be build with ``floo``.
...
hint how ``floo`` looks on the commandline, refer to the :ref:`quickstart`.

What is ``floo`` not?
---------------------

``floo`` tries to focus on being up conceptually simple, by hiding a lot of
...
``floo`` is a »general purpose« tool for file synchronization it of course
...
* `GitHub Issue Tracker <https://flooo/issues>`_: All things like bug reports or feature requests.
* The matrix chat room ``#floo`` on ``matrix.org``. Just `pick a client <https://matrix.org/docs/projects/try-matrix-now.html>`_ and join the room or click `this link <https://riot.im/app/#/room/#floo:matrix.org>`_ directly.
...
With that being said, ``floo`` is near a somewhat usable state where you can play around
...
At this moment ``floo`` is **only tested on Linux**. Porting and testing
